...
Canonical in-repo copy of the implementation plan (intent). Frozen except dated addenda. What actually shipped is recorded in [AS_BUILT.md](AS_BUILT.md). The original brief remains at [ClaimSight-Capstone-Design.md](../ClaimSight-Capstone-Design.md). Architecture diagrams: [high-level](architecture/claimsight-high-level-architecture.mermaid), [orchestration](architecture/claimsight-agent-orchestration.mermaid).
